...
#define SIZE 14 /* Max amount of values stored in each array */
int reallocations = 1; /* Keeps track of the amount of times we reallocate */
...
typedef struct multimap_value {
    int array[SIZE]; /* Keeps an aray of size 4 for the given key */
...
struct multimap {
    ...
    /* Points to the start of the allocated block of space */
    multimap_node *first;

    /* Points to the first free space in the allocated block of space */
    multimap_node *first_free;

    /* Points to the end of the allocated block of space */
    multimap_node *last;
};
...
multimap_node * alloc_mm_node2(multimap *mm);

multimap_node * find_mm_node(multimap_node *root, int key,
                             int create_if_not_found);

multimap_node * find_mm_node2(multimap *mm, multimap_node *root, int key,
                             int create_if_not_found);
...
//
multimap_node * alloc_mm_node2(multimap *mm) {
    if (mm->first_free >= mm->last) {
        mm->first = (multimap_node *) realloc((void *) mm->first, 
            10000 * sizeof(multimap_node) + (5000 * reallocations * 
                sizeof(multimap_node)));
        mm->last = mm->first + 10000 * sizeof(multimap_node) + (5000 * 
                reallocations * sizeof(multimap_node));
        mm->first_free = mm->first;
        reallocations++;
    }

    multimap_node *node = mm->first_free;
    mm->first_free = mm->first_free + sizeof(multimap_node);
    bzero(node, sizeof(multimap_node));
...
/* We create a new function similar to find_mm_node to use our new
alloc_mm_node2 funciton// */
multimap_node * find_mm_node2(multimap *mm, multimap_node *root, int key,
                             int create_if_not_found) {
    multimap_node *node;
    ...
    while (1) {
        ...
        if (node->key > key) {   /* Follow left child */
            if (node->left_child == NULL && create_if_not_found) {
                /* No left child, but caller wants us to create a new node. */
                multimap_node *new = alloc_mm_node2(mm);
                new->key = key;
...
multimap * init_multimap() {
    ...
    mm->first = malloc(5000 * sizeof(multimap_node));
    mm->last = mm->first + (5000 * sizeof(multimap_node));
    mm->first_free = mm->first;
    return mm;
}
...
void mm_add_value(multimap *mm, int key, int value) {
    ...
    if (node->values_tail != NULL) {
        if (node->values_tail->index < SIZE) {
            /* If we still have space in array, fill it up */
            node->values_tail->array[node->values_tail->index] = value;
            node->values_tail->index++;
        }
        else { /* If there's no space in tail's array */
            new_value = malloc(sizeof(multimap_value));
            /* Create more space */
            node->values_tail->next = new_value;
            node->values_tail = node->values_tail->next; /* Advance tail */
            node->values_tail->index = 0; /* Reset index */
            node->values_tail->array[node->values_tail->index] = value;
            node->values_tail->index++;
    ...
    else { /* If linked list of value arrays is empty */
        new_value = malloc(sizeof(multimap_value));
        node->values = new_value;
        node->values->index = 0;
        node->values->array[node->values->index] = value;
